...
const TEXT_ATTS = ["fontFamily", "fontSize", "fontWeight", "textAnchor", "dx", "dy"];
...
function fixTextYPosition(y, node) {
    if (node.attributes) {
        const fontSizeAttr = Object.keys(node.attributes).find(a => node.attributes[a].name === 'font-size');
        if (fontSizeAttr) {
            return '' + (parseFloat(y) - parseFloat(node.attributes[fontSizeAttr].value) - 3);
        }
    }
    if (!node.parentNode) {
        return y;
    }
    return fixTextYPosition(y, node.parentNode)
}
...
class SvgUri extends Component {
    ...
    createSVGElement(node, childs) {
        ...
        switch (node.nodeName) {
            ...
            case "text":
                ...
                if (componentAtts.y) {
                    componentAtts.y = fixTextYPosition(componentAtts.y, node);
                }
    ...
    obtainComponentAtts({nodeName, attributes}, enabledAttributes) {
        const styleAtts = {};
